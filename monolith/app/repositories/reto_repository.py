...
def get_retos() -> list:
    retos = challenges_collection.find({"is_deleted": False})
    return list(retos)


# Obtener reto por ID
def get_reto_by_id(reto_id: str) -> dict | None:
    reto = challenges_collection.find_one({
        "_id": ObjectId(reto_id),
        "is_deleted": False
    })

    if reto:
        reto["_id"] = str(reto["_id"])
...
def update_reto(reto_id: str, update_data: dict) -> dict | None:
    ...
    result = challenges_collection.find_one_and_update(
        {
            "_id": ObjectId(reto_id),
            "is_deleted": False
        },
